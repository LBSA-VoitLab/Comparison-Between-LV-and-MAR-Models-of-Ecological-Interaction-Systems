...
for (i in 1:30)
	{
	n_t = log(Ni)
	n_tplus1 = r + (A) %*% n_t + rand_on * rnorm(n=4, mean = rand_mean, sd = rand_sd)
...
MAR_artData1_noisy_ln = ni_store
MAR_artData1_noisy = exp(ni_store)

for (ii in 1:4) { points (MAR_artData1_noisy[,ii],col=varColor[ii]) }

# setwd('C:\\Users\\dolivenca3\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\11_Frontiers\\V3\\paper_scripts\\Fig_2_S9')
...
smoother_out = Smoother(
				...
				data1_spline2 = 2, 
				smooth_type = 1,
				df = c(15,15,15,15), # c(20, 15, 15, 12),	
				log_spline = TRUE
				)
plot(1,1,col='white')
...
#		)
# "100 % complete || DF = 15 15 15 15  ||  2 4 7 22 27 1163.505  ||  2 3 4 7 10 885.051"
# "100 % complete || DF = 20 15 15 12  ||  2 4 7 22 28 1530.054  ||  2 3 6 7 11 795.266"

( est1 = LV_pars_finder(
				smooth_out = smoother_out,
				alg1_lm2 = 1, 
				data_sample_alg = c(2, 4, 7, 22, 27)	#c(2, 4, 7, 22, 28)		
				) )
cbind(est1)
est1_mat = Format_pars(truePars = est1)
splineStart = smoother_out$splines_est[1,2:5];names(splineStart) = c('x1','x2','x3','x4')
out_est1 = solveLV(times = seq(1,31,.1), pars = est1_mat, initState = splineStart, equations = Equations)
...
sum( (MAR_artData1-out_est1[seq(1,301,10),2:5])^2 )
...
MAR_artData1_noisy_moretime = matrix(rep(NA,301*4),ncol=4 )
MAR_artData1_noisy_moretime[seq(1,301,10),] = MAR_artData1_noisy

MARest_orig1 =  fastMAR(
			data_vars = MAR_artData1_noisy_moretime,
			log_transform = FALSE,
			demeaned = FALSE,
			abstol_= 0.01,
			maxit_ = 1500,
...
for (iii in 1:4)
	...
	points(MAR_artData1_noisy[,iii],pch=20,col=varColor[iii])
	if (iii==1) { legend(15,80,legend=c('data','noisy data'),lty=c(1,NA),lwd=c(3,NA),pch=c(NA,20),col=c('lightgrey',varColor[ii]),bty = "n") }
	points(seq(1,31,.1),MARest_orig1$MAR_TS_Est[,iii] ,type='l',col='green',lwd=1)
	points(seq(1,31,.1),MARest_log1$MAR_TS_Est[,iii] ,type='l',col='green',lty=2,lwd=1)
	points(meanMaker(MARest_orig1)[iii,],type='l',col='purple',lty=1)
	points(meanMaker(MARest_log1)[iii,],type='l',col='purple',lty=2)
	}

sum( (MAR_artData1-MARest_orig1$MAR_TS_Est[seq(1,301,10),])^2 )
sum( (MAR_artData1-MARest_log1$MAR_TS_Est[seq(1,301,10),])^2 )
...
TS_smooth_data_moretime = matrix(rep(NA,301*5),ncol=5 )
TS_smooth_data_moretime[seq(1,301,10),] = TS_smooth_data

# MAR estimates without data log transformation, with spline smooth data  
MARest_orig1_smoothData =  fastMAR(
			data_vars = TS_smooth_data_moretime[,2:5],
			log_transform=FALSE,
			demeaned=FALSE,
			abstol_=0.01,
			maxit_ = 2000,
...
sum( (MAR_artData1-MARest_orig1_smoothData$MAR_TS_Est[seq(1,301,10),])^2 )
sum( (MAR_artData1-MARest_log1_smoothData$MAR_TS_Est[seq(1,301,10),])^2 )
...
for (iii in 1:4)
	...
	points(seq(1,31,.1),MARest_orig1$MAR_TS_Est[,iii] ,type='l',col='green',lwd=1)
	points(seq(1,31,.1),MARest_log1$MAR_TS_Est[,iii] ,type='l',col='green',lty=2,lwd=1)
	points(seq(1,31,.1),MARest_orig1_smoothData$MAR_TS_Est[,iii] ,type='l',col='orange',lwd=1)
	points(seq(1,31,.1),MARest_log1_smoothData$MAR_TS_Est[,iii] ,type='l',col='orange',lty=2,lwd=1)
	points(out_est1[,1],out_est1[,iii+1],type='l',col='cyan',lwd=3)
	}
...
#############################################################################
##### clustered dataset (noisy data 4) - 15 points were chosen from the artificial data and for each point five observations were created multiplying it by a random normal of mean 1 and variance 0.5

MAR_artData1_withTime = cbind(1:dim(MAR_artData1)[1],MAR_artData1)

set.seed(100)
( MAR_artData1_forClust = MAR_artData1_withTime[c(1:8,11,14,17,20,23,26,29),] )  #  #seq(0,101,10) #c(1,6,11,18,26,41,seq(51,101,10))

Clustered_MAR = function()
	{
	### population component control ###
	# initial conditions
	N1_init = 5
...
MAR_artData1_clustered = reps_TS[[1]][c(1:8,11,14,17,20,23,26,29),]
for (i in 2:5)
	{
	MAR_artData1_clustered = rbind(MAR_artData1_clustered,reps_TS[[i]][c(1:8,11,14,17,20,23,26,29),])
	}
rownames(MAR_artData1_clustered)=NULL
colnames(MAR_artData1_clustered) = c('t','X1','X2','X3','X4')
MAR_artData1_clustered = MAR_artData1_clustered[order(MAR_artData1_clustered[,1],decreasing=FALSE),]

# setwd('C:\\Users\\dolivenca3\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\11_Frontiers\\V3\\paper_scripts\\Fig_2_S8')
# save(reps_TS, file = "MAR_artData1_clustered_reps_TS.Rdata")
load(file = "MAR_artData1_clustered_reps_TS.Rdata")
...
for (ii in 1:length(Ni))
	{
	plot(MAR_artData1[,ii],type = 'l',lwd=3,col='lightgrey',xlim=c(0,dim(ni_store)[1]+.1*dim(ni_store)[1]),ylim=c(0,80),xlab='t',ylab=varNames[ii])
	for (i in 1:5) {lines(reps_TS[[i]][,1],reps_TS[[i]][,ii+1])}
	points(MAR_artData1_clustered[,1],MAR_artData1_clustered[,ii+1],pch=20,col=varColor[ii])
	points(MAR_artData1[,ii],type = 'l',lwd=3,col='lightgrey',xlim=c(0,dim(ni_store)[1]+.1*dim(ni_store)[1]),ylim=c(0,80),xlab='t',ylab=varNames[ii])
...
MAR_artData1_clustered_means = MAR_artData1_clustered[1:15,]
index = c(1:8,11,14,17,20,23,26,29)
for (i in 1:15)
	{
	MAR_artData1_clustered_means[i,] = apply(MAR_artData1_clustered[MAR_artData1_clustered[,1]==index[i],],2,mean)
	}
...
#		randomSearchTries = 100
#		)
#  "100 % complete || DF = 11 11 11 11  ||  2 3 4 5 12 216.54  ||  2 3 6 7 12 215.632"

( est1 = LV_pars_finder(
				smooth_out = smoother_out,
				alg1_lm2 = 1, 
				data_sample_alg = c(2,3,4,5,12) # c(5,6,7,8,9)		#c(3, 7, 10, 14, 22)		
				) )
cbind(est1)
est1_mat = Format_pars(truePars = est1)
splineStart = smoother_out$splines_est[1,2:5];names(splineStart) = c('x1','x2','x3','x4')
out_est2 = solveLV(times = seq(1,31,.1), pars = est1_mat, initState = splineStart, equations = Equations)
...
# SSE
sum( ( MAR_artData1 - out_est2[seq(1,301,10),2:5] )^2 )
...
TS_MAR_artData1_clustered_means = cbind(seq(1,31,0.1),matrix(rep(NA,301*4),ncol=4))
TS_MAR_artData1_clustered_means[c(1,11,21,31,41,51,61,71,101,131,161,191,221,251,281),] = MAR_artData1_clustered_means
...
# SSE
sum( ( MAR_artData1 - MARest_orig2$MAR_TS_Est[seq(1,301,10),] )^2 )
...
# SSE
sum( ( MAR_artData1 - MARest_log2$MAR_TS_Est[seq(1,301,10),] )^2 )
...
for (iii in 1:4)
	...
	points(MAR_artData1_clustered[,1],MAR_artData1_clustered[,iii+1],pch=20,col=varColor[iii])
	if (iii==1) { legend(15,80,legend=c('data','noisy data'),lty=c(1,NA),lwd=c(3,NA),pch=c(NA,20),col=c('lightgrey',varColor[ii]),bty = "n") }
	points(seq(1,31,.1),MARest_orig2$MAR_TS_Est[,iii] ,type='l',col='green',lwd=1)
	points(seq(1,31,.1),MARest_log2$MAR_TS_Est[,iii] ,type='l',col='green',lty=2,lwd=1)
	points(seq(1,31,.1),meanMaker(MARest_orig2)[iii,],type='l',col='purple',lty=1)
	points(seq(1,31,.1),meanMaker(MARest_log2)[iii,],type='l',col='purple',lty=2)
	}
...
smooth_TS_data_moretime = cbind(seq(1,31,0.1),matrix(rep(NA,301*4),ncol=4))
smooth_TS_data_moretime[seq(1,281,10),] = smooth_TS

# MAR estimates without data log transformation, with spline smooth data  
MARest_orig2_smoothData =  fastMAR(
			data_vars = smooth_TS_data_moretime[,2:5],
			log_transform=FALSE,
			demeaned=TRUE,
			abstol_=0.01,
			maxit_ = 1000,
...
sum( (MAR_artData1-MARest_orig2_smoothData$MAR_TS_Est[seq(1,301,10),])^2 )
sum( (MAR_artData1-MARest_log2_smoothData$MAR_TS_Est[seq(1,301,10),])^2 )
...
# setwd("C:\\Users\\dolivenca3\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\11_Frontiers\\V3\\paper_scripts\\Fig_2_S8")     # P51
...
for (i in 1:4)
	...
	points(seq(1,31,.1),MARest_orig1$MAR_TS_Est[,i] ,type='l',col=colorPallet[4],lwd=3)
	points(seq(1,31,.1),MARest_log1$MAR_TS_Est[,i] ,type='l',col=colorPallet[5],lwd=3)
	points(seq(1,31,.1),MARest_orig1_smoothData$MAR_TS_Est[,i] ,type='l',col='orange',lwd=3)
	points(seq(1,31,.1),MARest_log1_smoothData$MAR_TS_Est[,i] ,type='l',col='khaki',lwd=3)
...
for (i in 1:4)
	...
	points(seq(1,31,.1),MARest_orig2$MAR_TS_Est[,i] ,type='l',col=colorPallet[4],lwd=3)
	points(seq(1,31,.1),MARest_log2$MAR_TS_Est[,i] ,type='l',col=colorPallet[5],lwd=3)
	points(seq(1,31,.1),MARest_orig2_smoothData$MAR_TS_Est[,i] ,type='l',col='orange',lwd=3)
	points(seq(1,31,.1),MARest_log2_smoothData$MAR_TS_Est[,i] ,type='l',col='khaki',lwd=3)
...
colnames(SSEs_MAR) = c('Noisy MAR','Replicate MAR') 
rownames(SSEs_MAR) = c('ALVI-MI','MAR','MAR log transform','MAR with smoothing','MAR log transform with smoothing')
SSEs_MAR
...
smoother_out = Smoother(
				...
				data1_spline2 = 2, 
				smooth_type = 1,
				df = c(25,14,18,20),		#c(14,15,11,12), 
				log_spline = TRUE
				)
plot(1,1,col='white')
legend(1,1,legend = c('data','splines','slopes'),pch = c(20,NA,NA),lty = c(0,1,1),col = c('grey','green','lightgreen'),bty = "n")
